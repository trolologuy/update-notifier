...
def check_prerequisites():
    ...
    if not sys.version_info.major == 3 and sys.version_info.minor >= 6:
        ...
        print("Please use: python3 ./main.py")
        sys.exit(2)
...
def get_arg(argv):
    ...
    if "." not in inputfile:
        print("Error: Missing the file extension for the JSON file passed as argument.\nUsage: main.py -i <JSON file>")
        sys.exit(2)
...
def pfsense():
    ...
    if result:
        ...
        print("Error: The current version of the unifi cloudkey controller could not be retrieved !")
        return(None)
...
def main(inputfile):
    ...
    if  latest_synology_dsm_version == installed_synology_dsm_version:
        ...
        message(SYNOLOGY_DSM, False, latest_synology_dsm_version, installed_synology_dsm_version)
        up_to_date = False
        programs.append(SYNOLOGY_DSM)
    if  latest_synology_plex_version == installed_synology_plex_version:
        message(PLEX, True, '', '')
    else:
        message(PLEX, False, latest_synology_plex_version, installed_synology_plex_version)
        up_to_date = False
